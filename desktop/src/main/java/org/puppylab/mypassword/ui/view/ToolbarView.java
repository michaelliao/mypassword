...
public class ToolbarView {
    ...
    private Consumer<ItemType> onAddNew;
    ...
    public ToolbarView(Composite parent) {
        ...
        btnLock.addListener(SWT.Selection, e -> { if (onLock != null) onLock.run(); });
    }

    // ── public API ────────────────────────────────────────────────────

    public void setOnAddNew(Consumer<ItemType> listener) {
    ...
    private void addMenuItem(Menu menu, String label, ItemType type) {
        MenuItem item = new MenuItem(menu, SWT.PUSH);
        item.setText(label);
        item.addListener(SWT.Selection, e -> { if (onAddNew != null) onAddNew.accept(type); });
    }
}
