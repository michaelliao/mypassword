...
public class ItemListView {
    ...
    private static final Category[] CATEGORIES      = {
        Category.ALL, Category.FAVORITES, Category.LOGINS,
        Category.NOTES, Category.IDENTITIES, Category.TRASH
    };
    private static final String[]   CATEGORY_LABELS = {
        "All Items", "Favorites", "Logins", "Notes", "Identities", "Trash"
    };
    ...
    // ── state ─────────────────────────────────────────────────────────
    private List<VaultItem> allItems        = List.of();
    private Category        currentCategory = Category.ALL;

    // ── listeners ─────────────────────────────────────────────────────
    private Consumer<VaultItem> onSelectionChanged;
    ...
    public void setAllItems(List<VaultItem> items) {
        this.allItems = items;
        refreshItemTable();
    }
    ...
    public void setOnSelectionChanged(Consumer<VaultItem> listener) {
    ...
    private Table buildCategoryTable(Composite parent) {
        ...
        table.addListener(SWT.Selection, e -> {
            ...
            refreshItemTable();
            if (onCategoryChanged != null) onCategoryChanged.accept(currentCategory);
        });
    ...
    private Table buildItemTable(Composite parent) {
        ...
        col.setWidth(240);
        table.addControlListener(ControlListener.controlResizedAdapter(
                e -> col.setWidth(table.getClientArea().width)));
        ...
        table.addListener(SWT.Selection, e -> {
            ...
            int idx = table.getSelectionIndex();
            VaultItem item = idx >= 0 ? (VaultItem) table.getItem(idx).getData() : null;
    ...
    /**
     * Suppress the default foreground text (we draw everything in PaintItem).
     * When the row is selected, draw our own highlight background and clear the
    ...
    /**
     * Draw one row: coloured icon square on the left, title above and
     * subtitle below on the right.
     *
     *  ┌─────────────────────────────────────┐
     *  │  [L]  title                         │  ← TITLE_Y_OFF from row top
     *  │       subtitle (dimmed)             │  ← SUB_Y_OFF from row top
     *  └─────────────────────────────────────┘
     */
    private void onPaintItem(Event e) {
        VaultItem item = (VaultItem) ((TableItem) e.item).getData();
        if (item == null) return;
        ...
        Color  iconBg  = iconColor(item.type());
        ...
        String letter = iconLetter(item.type());
        Point  sz     = gc.textExtent(letter);
        gc.drawText(letter,
                e.x + ICON_LEFT + (ICON_SIZE - sz.x) / 2,
                iconY           + (ICON_SIZE - sz.y) / 2,
                true);

        // ── title ─────────────────────────────────────────────────────
        gc.setForeground(display.getSystemColor(
                selected ? SWT.COLOR_LIST_SELECTION_TEXT : SWT.COLOR_LIST_FOREGROUND));
        gc.drawText(item.title() != null ? item.title() : "",
                e.x + TEXT_LEFT, e.y + TITLE_Y_OFF, true);

        // ── subtitle ──────────────────────────────────────────────────
        gc.setForeground(display.getSystemColor(
                selected ? SWT.COLOR_LIST_SELECTION_TEXT : SWT.COLOR_DARK_GRAY));
        gc.drawText(item.subtitle() != null ? item.subtitle() : "",
                e.x + TEXT_LEFT, e.y + SUB_Y_OFF, true);
    ...
    private List<VaultItem> filtered() {
        return switch (currentCategory) {
            ...
            case LOGINS     -> allItems.stream().filter(i -> !i.deleted() && i.type() == ItemType.LOGIN).toList();
            case NOTES      -> allItems.stream().filter(i -> !i.deleted() && i.type() == ItemType.NOTE).toList();
            case IDENTITIES -> allItems.stream().filter(i -> !i.deleted() && i.type() == ItemType.IDENTITY).toList();
            case TRASH      -> allItems.stream().filter(VaultItem::deleted).toList();
        };
    }

    private Color iconColor(ItemType type) {
        return switch (type) {
            case LOGIN    -> loginColor;
            case NOTE     -> noteColor;
            case IDENTITY -> identityColor;
        };
    }

    private String iconLetter(ItemType type) {
        return switch (type) {
            case LOGIN    -> "L";
            case NOTE     -> "N";
            case IDENTITY -> "I";
